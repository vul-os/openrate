<!-- Generated from docs/api.md by `go run ./site/gen`. Do not edit this file — edit the source and regenerate. -->
# API reference
...
| `GET /readyz` | Readiness — a conversion would actually succeed |
...
pair it asks about. Use `/readyz` to decide when to send the first
request. Every managed sidecar in [`sdks/`](https://github.com/vul-os/openrate/blob/main/sdks) made exactly this mistake
...
[rate limiter](#configuration) never applies to it: polling readiness cannot
