...
// WHY
// ───
// openrate tagged and published 0.1.8 while its Rust crate still said 0.1.0 and
// most of the rest said 0.1.2 — bun, deno, node (and its lockfile), java,
// dotnet, python, ruby.
...
// literal, it is a check — the same conclusion this repo reached for its C
...
const MANIFESTS = [
  ...
  { sdk: 'java', file: 'pom.xml', read: (s) => (/<artifactId>openrate<\/artifactId>\s*<version>([^<]+)<\/version>/.exec(s) || [])[1] ?? null },
  { sdk: 'dotnet', file: 'OpenRate.csproj', read: (s) => (/<Version>([^<]+)<\/Version>/.exec(s) || [])[1] ?? null },
  { sdk: 'rust', file: 'Cargo.toml', read: (s) => (/^\s*version\s*=\s*"([^"]+)"/m.exec(s) || [])[1] ?? null },
  { sdk: 'python', file: 'pyproject.toml', read: (s) => (/^\s*version\s*=\s*"([^"]+)"/m.exec(s) || [])[1] ?? null },
  { sdk: 'ruby', file: 'openrate.gemspec', read: (s) => (/\.version\s*=\s*"([^"]+)"/.exec(s) || [])[1] ?? null },
];
...
const NO_MANIFEST = {
  ...
  c: 'a header and a Makefile; the version lives in openrate-ffi/include/openrate.h, generated from VERSION',
...
function check(root) {
  ...
  // `--tag <tag>`, `openrate_<v>_source.zip`. patala does exactly that, and it
  ...
  // published to.
  //
  // The Python README said `pip install openrate`, which 404s. llmux's said
  ...
  for (const [name, text] of sdkDocs) {
    for (const { re, registry } of UNPUBLISHED) {
      for (const m of text.matchAll(re)) {
        ...
        // says "Not `pip install openrate`", and patala's says "There is no
...
if (process.argv.includes('--selftest')) {
  ...
  const cases = [
    ...
    { name: 'the pom version drifts', file: join(SDKS, 'java/pom.xml'), edit: (s) => s.replace(/(<artifactId>openrate<\/artifactId>\s*<version>)[^<]+/, '$19.9.9') },
