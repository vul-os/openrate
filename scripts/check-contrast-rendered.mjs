...
// Measured cold, 602 of 700 elements "fail" at 1:1 — they are not faded, they
// are not shown yet. That is a capture artifact, and this repo has been fooled
// by that exact artifact before.
//
// Emulating `prefers-reduced-motion: reduce` settles them, because the page
// honours it (`html.js .rv { opacity: 1 }`). It is also the honest thing to
// measure: it is a real setting on real machines, and the words have to be
// legible in it. A side effect worth naming — if that reduced-motion rule ever
...
const MIN_ELEMENTS_LANDING = 400;
const MIN_ELEMENTS_DOCS_CHAPTER = 20;
const MIN_DOCS_CHAPTERS = 10;
