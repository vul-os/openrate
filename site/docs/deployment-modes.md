<!-- Generated from docs/deployment-modes.md by `go run ./site/gen`. Do not edit this file — edit the source and regenerate. -->
...
[`sdks/README.md`](https://github.com/vul-os/openrate/blob/main/sdks/README.md).

## The decision, in order

**1. Is your program written in Go?**
Then use the [library](#library). There is no benefit to any other mode:
...
Packages for both of those live in [`sdks/`](https://github.com/vul-os/openrate/blob/main/sdks/README.md) — **bun, C,
...
Use the [C ABI](#c-abi). There are two reasons, and the second is the better
one.
...
gives it to you. See [Proving it sends nothing](#zero-network).
...
- **`-tags noui` only matters to a program that imports `serve`.** A host that
  ...
  either way. See [Proving it sends nothing](#zero-network) for how that is
...
- [Quickstart](#quickstart) — the first ten minutes in each mode
- [Embed as a Go library](#library)
- [Use it from another language](#c-abi)
- [Configuration](#configuration) — flags and environment for the binary
