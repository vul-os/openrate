// openrate DIRECT mode on Bun — libopenrate inside this process, over the C ABI.
//
//   bun run examples/direct.ts
//
// No permission flags: bun:ffi has no permission model, which is a real
...
// fetch. Part 2 builds a REFRESHER, which does. If this machine is offline,
// part 2 says so and the example still exits 0.

import { abiVersion, Engine, openHandles, resolveLibrary } from "../index.ts";
...
// ===========================================================================
{
  ...
  // bun:ffi has no async call mode, so refresh() runs here and freezes the loop.
  // The tick count below is how much of the event loop survived: zero.
  let ticks = 0;
  ...
  clearInterval(timer);
  console.log(`            blocked the event loop for ${Date.now() - began} ms; timer fired ${ticks}x`);
  ...
  // The non-blocking way to do the same thing: start() returns immediately
  // because the loop is a goroutine inside the library, so waiting for rates is
  // a Bun timer polling meta() rather than a frozen process.
  refresher.start();
  let polls = 0;
  while (engine.meta().currencies.length === 0 && polls < 40) {
    polls++;
    await Bun.sleep(250);
  }
  console.log(`start       loop running; ${engine.meta().currencies.length} currencies after ${polls} non-blocking polls`);
  refresher.stop();
}
