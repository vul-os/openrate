...
1. **The Go runtime lives in your process** — its GC, its scheduler, and its
   ...
   `SIGUSR2`). V8 uses `SIGSEGV` for its own write barriers, which is the
   overlap that matters. **`SIGPROF` is not touched**, so `--prof`, `0x` and
   other sampling profilers are unaffected.
