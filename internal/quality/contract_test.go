...
func TestAssessPublishesSelfConsistentFields(t *testing.T) {
	...
	p := graph.Pair{Rate: 18.5, Hops: 2, AsOf: now.Add(-20 * time.Hour), Sources: []string{"coinbase"}}
	quotes := []graph.Quote{
...
func TestCorroborationAgreeThresholdIs50Bps(t *testing.T) {
	...
	at := func(bps float64) (Corroboration, float64) {
		return corroborate([]graph.Quote{
			{Source: "a", Rate: 100},
...
func TestCorroborationBandsMatchDocumentation(t *testing.T) {
	...
	if c, f := corroborate([]graph.Quote{{Source: "a", Rate: 100}}); f != 0.88 || c.Agree {
...
func TestCaveatMultipliersMatchDocumentation(t *testing.T) {
	...
	perfect := func(from, to string) Assessment {
		p := graph.Pair{Rate: 1, Hops: 1, AsOf: now, Sources: []string{"ecb"}}
		q := []graph.Quote{{Source: "a", Rate: 100, Time: now}, {Source: "b", Rate: 100, Time: now}}
...
func TestDocumentedProvenanceTable(t *testing.T) {
	now := time.Date(2026, 6, 24, 12, 0, 0, 0, time.UTC)
	tight := []graph.Quote{ // 2 quotes ~16 bps apart (<= 25)
		{Source: "a", Rate: 18.50, Time: now},
		{Source: "b", Rate: 18.53, Time: now},
	}
	single := []graph.Quote{{Source: "a", Rate: 18.50, Time: now}}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.row, func(t *testing.T) {
			p := graph.Pair{Rate: 18.5, Hops: tc.hops, AsOf: now.Add(-tc.age), Sources: []string{tc.source}}
...
func TestGradeARequiresCorroboration(t *testing.T) {
	now := time.Date(2026, 6, 24, 12, 0, 0, 0, time.UTC)
	mk := func(age time.Duration, src string, quotes []graph.Quote) Assessment {
		p := graph.Pair{Rate: 18.5, Hops: 1, AsOf: now.Add(-age), Sources: []string{src}}
		return Assess("USD", "ZAR", p, quotes, now)
	}
	two := []graph.Quote{{Source: "a", Rate: 100, Time: now}, {Source: "b", Rate: 100.2, Time: now}}
	one := []graph.Quote{{Source: "a", Rate: 100, Time: now}}
	...
	p := graph.Pair{Rate: 18.5, Hops: 2, AsOf: now.Add(-20 * time.Hour), Sources: []string{"sarb"}}
