<!-- Generated from docs/graph-model.md by `go run ./site/gen`. Do not edit this file — edit the source and regenerate. -->
# The graph model
...
4. `as_of` is the **oldest** edge timestamp on the path — a cross rate is only as
   ...
   [the as-of contract](#api) for what that means for a consumer.
...
and has no reciprocal to protect. See [interest rates](#interest-rates).
...
other changes. See [SOURCES.md](#sources).
