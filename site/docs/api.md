...
|---|---|
| [`GET /api/v1/rates`]() | All currencies vs. a base |
| [`GET /api/v1/convert`]() | Convert an amount between two currencies |
| [`GET /api/v1/meta`]() | Sources, freshness, and currency list |
...
readiness probe (the [Go library](#library) waits on it during `Start`).
...
| `quality` | Grade (A–D), confidence, freshness, directness, corroboration, caveats — see [Accuracy](#accuracy) |
...
See [the graph model](#graph-model) for how `path`/`legs`/`hops` are chosen.
...
Full model, including every multiplier: [Accuracy & quality](#accuracy).
