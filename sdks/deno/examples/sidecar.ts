...
//     OPENRATE_READY_TIMEOUT_MS=3000 deno task example:sidecar
//
// NO_PROXY is not decoration: Deno's fetch honours HTTP(S)_PROXY for loopback
...
// waitForRates polls /readyz on a timer — nothing blocks while it waits.
...
const readyTimeoutMs = Number(Deno.env.get("OPENRATE_READY_TIMEOUT_MS") ?? 20_000);
let currencies = 0;
try {
  ...
  // this in CI sees success. Deno.exitCode, not Deno.exit(): the rest of the
  // script still runs and the child still gets killed on the way out.
  Deno.exitCode = 1;
}
if (currencies > 0) {
