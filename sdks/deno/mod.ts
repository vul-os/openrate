 ...
 * await side.waitForRates();                  // start() waited for the LISTENER
 * await side.convert("EUR", "ZAR", 100);
 * ```
 *
 * The waitForRates() line is not optional politeness: the startup refresh has
 * not finished when start() returns, and a convert against an empty book is a
 * 404 that reads like a bad currency code.
 *
...
function describeNotReady(text: string): string {
  ...
  const causes = (body.sources ?? []).flatMap((s) =>
    s.last_error ? [`${s.name ?? "?"}: ${s.last_error}`] : []
  );
 ...
 *
 * {@link start} waits for the listener (LIVENESS); {@link waitForRates} waits
 * for the book (READINESS). Both, in that order, before the first convert.
 *
...
export class Sidecar implements AsyncDisposable {
  ...
  static async start(opts: SidecarOptions = {}): Promise<Sidecar> {
    ...
    const child = new Deno.Command(binary, {
      args,
      // Merged over the inherited environment — `clearEnv` is false, so the
      // parent's variables still reach the child and paid-source API keys pass
      // through.
      //
      ...
      env: { OPENRATE_RATELIMIT: "0", ...opts.env },
      stdout: "inherit",
      stderr: "inherit",
    }).spawn();
    ...
    for (;;) {
      ...
      if (Date.now() > deadline) {
        ...
        throw new Error(
          `openrate did not start listening on ${addr} within ${seconds(opts.timeoutMs ?? 10_000)}`,
        );
      }
   ...
   * outcomes until then. It waits on a timer, so the isolate keeps serving.
   *
