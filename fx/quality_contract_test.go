...
func TestAssessPublishesSelfConsistentFields(t *testing.T) {
	...
	p := Pair{Rate: 18.5, Hops: 2, AsOf: now.Add(-20 * time.Hour), Sources: []string{"coinbase"}}
	quotes := []Quote{
...
func TestCorroborationBandsMatchDocumentation(t *testing.T) {
	...
	if c, f := corroborate([]Quote{{Source: "a", Rate: 100}}); f != 0.88 || c.Agree {
...
func TestCaveatMultipliersMatchDocumentation(t *testing.T) {
	...
	perfect := func(from, to string) Assessment {
		p := Pair{Rate: 1, Hops: 1, AsOf: now, Sources: []string{"ecb"}}
		q := []Quote{{Source: "a", Rate: 100, Time: now}, {Source: "b", Rate: 100, Time: now}}
...
func TestDocumentedProvenanceTable(t *testing.T) {
	now := time.Date(2026, 6, 24, 12, 0, 0, 0, time.UTC)
	tight := []Quote{ // 2 quotes ~16 bps apart (<= 25)
		{Source: "a", Rate: 18.50, Time: now},
		{Source: "b", Rate: 18.53, Time: now},
	}
	single := []Quote{{Source: "a", Rate: 18.50, Time: now}}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.row, func(t *testing.T) {
			p := Pair{Rate: 18.5, Hops: tc.hops, AsOf: now.Add(-tc.age), Sources: []string{tc.source}}
...
func TestGradeARequiresCorroboration(t *testing.T) {
	now := time.Date(2026, 6, 24, 12, 0, 0, 0, time.UTC)
	mk := func(age time.Duration, src string, quotes []Quote) Assessment {
		p := Pair{Rate: 18.5, Hops: 1, AsOf: now.Add(-age), Sources: []string{src}}
		return Assess("USD", "ZAR", p, quotes, now)
	}
	two := []Quote{{Source: "a", Rate: 100, Time: now}, {Source: "b", Rate: 100.2, Time: now}}
	one := []Quote{{Source: "a", Rate: 100, Time: now}}
	...
	p := Pair{Rate: 18.5, Hops: 2, AsOf: now.Add(-20 * time.Hour), Sources: []string{"sarb"}}
